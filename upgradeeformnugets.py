...
def main():
    ...
    if int(get_git_status()) > 0:
        ...
        os.chdir("ServiceOuterInnerResourcePlugin")
        packages = [
            'Microting.eForm', 'Microting.eFormApi.BasePn', 'Microting.eFormOuterInnerResourceBase'
        ]
        process_repository('ServiceOuterInnerResourcePlugin.csproj', packages, 'eform-service-outer-inner-resource-plugin')

        os.chdir("..")
        os.chdir("ServiceOuterInnerResourcePlugin.Integration.Test")
        packages = [
            'Microsoft.NET.Test.Sdk', 'NUnit', 'NUnit3TestAdapter', 'NUnit.Analyzers'
        ]
        process_repository('ServiceOuterInnerResourcePlugin.Integration.Test.csproj', packages, 'eform-service-outer-inner-resource-plugin')
        ...
        if new_number_of_commits > current_number_of_commits:
            ...
            os.chdir("..")
            run_command(f"github_changelog_generator -u microting -p eform-service-outer-inner-resource-plugin -t {os.getenv('CHANGELOG_GITHUB_TOKEN')}", shell=True)
